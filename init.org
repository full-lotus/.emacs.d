...
(defun pm--org-reveal-inside-source-block ()
  ...
  (interactive)
  (let* ((org? (eq 'org-mode major-mode))
				 (pm? (and (poly-org-mode?) (not org?))))
		(when org?
...
(defun pm--org-pre-bookmark-jump ()
  "switch to org mode before bm-next/previous"
  (let* ((org? (eq 'org-mode major-mode))
				 (pm? (and (poly-org-mode?) (not org?))))
		(when org?
		  (let ((post-command-hook nil)
...
(defun pm--org-bm-next ()
  "hooked version of bm-next"
  (interactive)
  (pm--org-pre-bookmark-jump)
  (call-interactively 'bm-next))
(defun pm--org-bm-previous ()
  "hooked version of bm-next"
  (interactive)
  (pm--org-pre-bookmark-jump)
  (call-interactively 'bm-previous))

(define-key poly-org-mode-map (kbd "<f2>") 'pm--org-bm-next)
(define-key poly-org-mode-map (kbd "S-<f2>") 'pm--org-bm-previous)
