  ...
  (let* ((org? (eq 'org-mode major-mode))
		...
		(when org?
		  (let ((post-command-hook nil)
				...
				;; Unfold parent headings if they're folded
				(let* ((old-line (line-number-at-pos))
               (new-line old-line))
			    (previous-line)
			    (setq new-line (line-number-at-pos))

	  ;; Parent heading is folded if previous-line skips lines
	  ;; FIXME: not actually correct
	  ;; It gives false-positives on sibling headings
	        (while (> 1 (- old-line new-line))
		        (next-line)
		        (org-show-children) ;; Same as folding to CONTENTS
