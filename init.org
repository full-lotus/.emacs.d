...
3. subtree expanded (everything visible)"
  (interactive)
  (cond
   ((not (org-at-heading-p)) (apply orig-fun args))
   ((eq (prefix-numeric-value current-prefix-arg) 4) (apply orig-fun args))
   (t (save-excursion
	(org-back-to-heading t)
	(save-restriction
	  (let ((beg (point))
		;; -1 is to not hide newline character before the next heading
		(end (- (org-end-of-subtree t t nil) 1)))
	    (narrow-to-region beg end)
	    (org-cycle-internal-global)))))))

(advice-add 'org-cycle :around 'org-cycle-current-subtree)
;; (advice-remove 'org-cycle 'org-cycle-current-subtree)
#+end_src
...
** Org-mode fold state bug fix (prevent unfolding)
The bug occurs when point moves in/out of source code blocks. The reason is
~move-overlay~ fn, which cuts out overlays from one of the buffers.

The issue can be avoided by setting ~org-fold-core-style 'text-properties~, but
I read that ~org-fold-core-style 'overlays~, is less buggy with poly-org. So I
decided to stick with this fix instead of changing the setting.

The fix is still WIP, I haven't worked with it extensively.

#+begin_src elisp :tangle ~/.emacs.d/elisp/polymode.tangled.el
(defun pm--move-overlays-hack (from-buffer to-buffer)
  "Delete all overlays in TO-BUFFER, then copy FROM-BUFFER overlays to it."
  (delete-all-overlays to-buffer)
;;   (message "\n\n-----------------------------
;; from-buffer %s to-buffer %s" from-buffer to-buffer)
  (with-current-buffer from-buffer
    (let ((overlays (overlays-in 1 (1+ (buffer-size)))))
      ;; (message "number of overlays %s" (length overlays))
      (mapc (lambda (o)
		...
		;; copying is necessary to preserve folding state in org-mode
		;; otherwise move-overlay removes overlays in from-buffer
		(let ((o-copy (copy-overlay o))
		      (inhibit-redisplay t)
		      (inhibit-modification-hooks t))
		  ;; (message "start %s end %s" (overlay-start o) (overlay-end o))
		  ;; (message "overlay-props %s\n" (overlay-properties o))
		  (move-overlay o-copy (overlay-start o) (overlay-end o)
				to-buffer))))
            overlays))))

(advice-add 'pm--move-overlays :override 'pm--move-overlays-hack)
#+end_src
...
(defun pm--tangle-inside-source-block ()
  "If we are inside poly-org source block, go to beginning of current buffer,
switch to org-mode, tangle, go back, and finally switch back to needed mode."
  (interactive)
  (cond
   ((not (poly-org-mode?)) nil)
   ((not (org-in-src-block-p t)) (org-babel-tangle))
   (t (let ((inhibit-redisplay t)
         (inhibit-modification-hooks t)
	 ;; we don't need any visuals (overlay switching) for this command
	 (pm--move-overlays-hack (lambda ()))
	 (curr-point (point)))
     (goto-char (point-min))
