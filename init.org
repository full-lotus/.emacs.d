...
#+begin_src elisp :tangle ./init.el
(server-start)
;; (setq server-temp-file-regexp nil)
#+end_src


** Emacs customization API
#+begin_src elisp :tangle ./init.el
...
#+begin_src elisp :tangle ~/.emacs.d/elisp/global-settings.tngl.el
;; Optimize for maximum runtime speed (not load time)

;; Garbage collection optimizations
(setq gc-cons-threshold 100000000       ;; 100MB - GC less frequently
      gc-cons-percentage 0.1)            ;; Low threshold for when to trigger GC

;; Font locking optimizations
(setq font-lock-maximum-decoration 2     ;; Reduce font-lock complexity
      jit-lock-defer-time 0.05           ;; Slight delay before font-locking
      jit-lock-context-time 0.5)          ;; Delay context font-locking

;; Don't recompute expensive properties on long lines
...
;; Don't compact font caches during operation
(setq inhibit-compacting-font-caches t)

;; Optimize flycheck - don't check on every keystroke
(setq flycheck-idle-change-delay 1.0      ;; Wait 1 second before checking
      flycheck-buffer-switch-check-delay 1.0)

;; Disable expensive features in large buffers
(defun my-disable-expensive-features-in-large-buffers ()
  (when (> (buffer-size) 100000)
    (setq-local jit-lock-contextually nil)
    (setq-local font-lock-multiline nil)))

(add-hook 'prog-mode-hook #'my-disable-expensive-features-in-large-buffers)

;; Reduce line number computation if using display-line-numbers
...
** File system
#+begin_src elisp :tangle ./init.el
(safe-load "~/.emacs.d/elisp/file-system.tngl.el")
#+end_src

*** Dired
#+begin_src elisp :tangle ~/.emacs.d/elisp/file-system.tngl.el
;; dired-subtree uses really dark background color, makes dirs unreadable
(dotimes (i 6)
  (set-face-background
   (intern (format "dired-subtree-depth-%d-face" (1+ i)))
   nil))
...
(defun pm--org-reveal-inside-source-block ()
  "Reveals only the parent header of a current org-mode source block"
  (interactive)
  (when (poly-org-mode?)
    (let* ((post-command-hook nil)
           (pm--move-overlays (lambda ()))
           (curr-point (point))
           (base-mode (buffer-local-value 'major-mode (pm-base-buffer)))
           (inner-chunk (not (eq major-mode base-mode))))
      (when inner-chunk
...
**** 80-char indicator with fill-column-indicator
#+begin_src elisp :tangle ~/.emacs.d/elisp/text-settings.tngl.el
...
**** custom theme
#+begin_src elisp :tangle ~/.emacs.d/elisp/text-settings.tngl.el
;simple face customization, comment if unwanted
(load-theme 'lavender-light)
#+end_src
*** Editing
...
(use-package markdown-mode
  ...
  (setq markdown-fontify-code-blocks-natively t
        ...
        markdown-gfm-additional-languages '("Clojure" "clojure"))
...
This function follows org-cycle rules but in the order FOLD -> CHILDREN -> CONTENTS -> ALL -> FOLD

Variables:
#+begin_src elisp :tangle ~/.emacs.d/elisp/org-mode.tngl.el
(defvar my-org-fold-state 0)
(defcustom custom-unfold-contents-level 2
  "how many levels of headings to see from unfolding to CONTENTS"
  :type 'integer
  :group 'org-cycle)
#+end_src

Utility functions (for cleanness):
#+begin_src elisp :tangle ~/.emacs.d/elisp/org-mode.tngl.el
;; Utility functions
(defun my-org-folded ()
...
(defun my-org-foldp ()
  "Copied org-cycle-internal-local for properly detecting fold state."
...
(defun my-org-custom-fold (&optional arg)
  (interactive)
  (unless (number-or-marker-p arg)
		(setq arg 1))
  (save-excursion
    (labels ((inc () (setq my-org-fold-state (mod
																	      (+ my-org-fold-state arg)
																	      4))))
      (org-back-to-heading)
      (unless (eq this-command last-command)
				(setq my-org-fold-state
	            (if (my-org-foldp)
							    (if (> arg 0) 1 3)
								0)))

      (my-org-folded) ;; Common state to unfold from
      (case my-org-fold-state
				...
				(2 ;; at CHILDREN
				 ;; Contents
				 (org-show-children custom-unfold-contents-level)
				 (inc)
				 (message "CONTENTS"))
				(3 ;; at CONTENTS
				 ;; Show All
				 (org-fold-region (point)
											    (save-excursion (org-next-visible-heading 1)
																			    (goto-char (1- (point))))
											    nil '(block drawer headline))
				 (inc)
				 (message "ALL"))))))

(defun my-org-custom-fold-backwards ()
  (interactive)
  (setq this-command 'my-org-custom-fold)
  (my-org-custom-fold -1))
#+end_src

Keybinds:
#+begin_src elisp :tangle ~/.emacs.d/elisp/org-mode.tngl.el
(define-key org-mode-map (kbd "C-<wheel-up>") 'my-org-custom-fold)
(define-key org-mode-map (kbd "C-<wheel-down>") 'my-org-custom-fold-backwards)
...
(setq org-todo-keyword-faces
	...
	("ABANDONED" . (:foreground "grey" :weight bold))
...
**** Clojure defn search
Plain org-mode file links find the first occurrence of the
search text anywhere in the file.  This causes links like
=[[file:my.clj::multiply-two]]= to land on docstrings or
comments mentioning the function name instead of the
=\(defn multiply-two ...)= definition.

This hook intercepts org file link searches for =.clj=,
=.cljs=, and =.cljc= files, finding the exact =\(defn=
or =\(defn-= form at the start of a line.  The regex uses
symbol-boundary matching so =add-two= does not match
=add-two-old=.  Falls through to default org search if no
=defn= is found, for file types other than Clojure, or for
function names not at top-level =defn=/=defn-= forms.

#+begin_src elisp :tangle ~/.emacs.d/elisp/org-mode.tngl.el
(defun org-clj-defn-search (search-string)
  "Handle org file links to Clojure defn definitions.
When opened file is .clj/.cljs/.cljc, search for a top-level
\(defn, \(defn-, \(e/defn, or \(e/defn- form using symbol-boundary
matching.  Returns non-nil on success, nil to fallthrough."
  (let ((filename (buffer-file-name)))
    (when (and filename (string-match-p "\\.clj[sc]?\\'" filename))
      (let ((regex (concat "^\\((defn-?\\|(e/defn-?\\)[[:space:]]+"
...
(add-hook 'org-execute-file-search-functions #'org-clj-defn-search)
#+end_src
...
*** 80-char indicator in polymode code blocks
#+begin_src elisp :tangle ~/.emacs.d/elisp/polymode.tngl.el
;; Enable fill-column indicator in polymode inner buffers (code blocks)
(add-hook
 'polymode-init-inner-hook
 (lambda ()
