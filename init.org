...
**** Why not polymode (poly-org)?
I spent 2 days trying to get polymode with poly-org to work, but it was ultra
shitty, and broke everything left and right, sometimes silently. I thought had
found a workaround, but I was wrong. See [[https://github.com/polymode/poly-org/issues/53][issue]].
...
(defun tangle-on-save-org-mode-file ()
  (when (and (string-match-p
    ...
    (message
     "tangle hook, pm-base-buffer-before-save %s" pm-base-buffer-before-save)
    (message "tangle hook, pm-base-buffer %s" (pm-base-buffer))
    (message "tangle hook, current buffer %s" (current-buffer))
    (org-babel-tangle)
    ))
(add-hook 'after-save-hook 'tangle-on-save-org-mode-file)
(remove-hook 'after-save-hook 'tangle-on-save-org-mode-file)
;; (add-hook 'before-save-hook 'tangle-on-save-org-mode-file)
;; (remove-hook 'before-save-hook 'tangle-on-save-org-mode-file)
...
(advice-remove 'org-babel-tangle 'org-babel-tangle-no-buffer-pop-up)
...
(defvar pm-base-buffer-before-save)

(defun pm-store-base-buffer-before-save (&rest _)
  ""
  (setq pm-base-buffer-before-save (pm-base-buffer))
  (message "pm-base-buffer-before-save %s" pm-base-buffer-before-save))

(advice-add 'save-buffer :before 'pm-store-base-buffer-before-save)
(advice-remove 'save-buffer 'store-pm-base-buffer-before-save)

(defun pm-move-point-to-start-before-tangle (orig-fun &rest args)
  ""
  (let ((orig-buffer (current-buffer)))
    (message "pm-base-buffer %s" (pm-base-buffer))
    (switch-to-buffer pm-base-buffer-before-save)
    (save-excursion
      (goto-char (point-min))
      (apply orig-fun args))
    (switch-to-buffer orig-buffer)))

(advice-add 'org-babel-tangle :around 'pm-move-point-to-start-before-tangle)
;; (advice-remove 'org-babel-tangle 'pm-move-point-to-start-before-tangle)
#+end_src
...
(remove-hook 'after-save-hook 'to-agenda-on-save-org-mode-file)
